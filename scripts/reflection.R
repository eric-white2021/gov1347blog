####### THIS IS PRIMARILY AN EXTENSION OF MY FINAL MODEL R CODE; SKIP TO LINE 564 FOR NEW CODE FOR THIS REFLECTION
...
data_needed <- comb_2020 %>%
  mutate(D_pv2p = 100*democrat/(democrat + republican),
         R_pv2p = 100*republican/(democrat + republican),
         margin = D_pv2p - R_pv2p) %>%
  select(state, democrat, republican, final_2020, D_pv2p, R_pv2p, margin, electoral, Dwin)
...
lost_values <- pvstate %>% filter(year == 2016 & state%in%lost_states) %>% mutate(difference = D_pv2p - R_pv2p) %>%select(state, D_pv2p, R_pv2p, difference)
found_values <- pv2p %>% select(state, D_pv2p, R_pv2p, difference)

all_values <- rbind(lost_values, found_values) %>% arrange(state)

data_needed$state[abs(data_needed$margin) < 8]

all_values$state[abs(all_values$difference) > 10]
...
pvstate_2020 <- read_csv("popvote_bystate_1948-2020.csv")

true_voting <- pvstate_2020 %>%
  filter(year == 2020) %>%
  mutate(margin = 100*(D_pv2p - R_pv2p)) %>%
  select(state, D_pv2p, R_pv2p, margin)

data_needed <- data_needed %>% rename(difference = margin) %>% select(state, D_pv2p, R_pv2p, difference)


discrepancy <- data_needed %>%
  left_join(true_voting, by="state") %>%
  rename(pred_Dpv2p = D_pv2p.x,
         pred_Rpv2p = R_pv2p.x,
         real_Dpv2p = D_pv2p.y,
         real_Rpv2p = R_pv2p.y) %>%
  mutate(real_Dpv2p = 100*real_Dpv2p,
         real_Rpv2p = 100*real_Rpv2p,
         error = abs(difference) - abs(margin),
         biden_error = pred_Dpv2p - real_Dpv2p,
         trump_error = pred_Rpv2p - real_Rpv2p)

close_states <- data_needed$state[abs(data_needed$difference) < 8]  

plot_usmap(data = discrepancy, regions = "states", values = "biden_error", include=close_states) + 
  scale_fill_gradient2(
    high = "dodgerblue2", 
    mid = "white",
    low = "firebrick2",
  ...
  theme_void() + 
  labs(title = "Error between my model and the actual result", subtitle = " Blue means my model overpredicted in favor of Democrats \n\ Red means my model overpredicted in favor of Republicans")


RMSE <- sqrt(mean(discrepancy$biden_error^2))

plot_usmap(data = discrepancy, regions = "states", values = "error") + 
  scale_fill_gradient2(
    high = "green", 
    mid = "white",
    low = "red",
  ...
  theme_void() + 
  labs(title = "Error between my model and the actual result", subtitle = " Green means my model overpredicted the difference in vote share, \n\ red means my model underpredicted the difference in vote share")
